...
import Data.Scientific (fromFloatDigits, isInteger, toBoundedInteger, toRealFloat)
...
jsFreeValue ctx val = with val $ \v ->
  [C.block| void {
...
jsonToJSValue ctx (Number n) =
  if not (isInteger n)
    then liftIO $ jsNewFloat64 ctx (toRealFloat n)
...
jsonToJSValue ctxPtr (Array xs) = do
  ...
  flip imapM_ xs $ \index value -> do
    ...
    code <-
      liftIO
        ( with arrVal $ \arrValPtr -> with val $ \valPtr ->
...
jsonToJSValue ctxPtr (Object o) = do
  ...
  forM_ (toList o) $ \(key, value) -> do
    ...
    code <-
      liftIO
        ( with objVal $ \objValPtr -> with val $ \valPtr ->
...
jsObjectToJSON ctxPtr obj = do
  ...
  plen <-
    jsGetOwnPropertyNames ctxPtr obj properties flags
  ...
  res <-
    collectVals properties objPtr 0 plen
      `catch` ( \(e :: SomeJSRuntimeException) -> do
                  liftIO $ free objPtr
                  throwM e
              )
